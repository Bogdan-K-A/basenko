...
const formatDateUA = (dateString) => {
  if (!dateString) return "20 жовтня"; // fallback
...
const Faq = () => {
  ...
  const [startDate, setStartDate] = useState("20 жовтня"); // дефолтная дата

  // Загрузка даты из timer.json
  useEffect(() => {
    const loadStartDate = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.targetDate) {
            const formattedDate = formatDateUA(data.targetDate);
            setStartDate(formattedDate);
          }
        }
      } catch (error) {
        console.error("Ошибка загрузки даты для FAQ:", error);
      }
    };

    loadStartDate();
    // Проверяем обновления каждые 30 секунд (синхронизация с таймером)
    const interval = setInterval(loadStartDate, 30000);
  ...
  const faqData = [
    {
      ...
      answer: `Старт нового потоку — ${startDate}. Тривалість програми — 2 тижні, із тренуваннями 4 рази на тиждень.`,
    },
    {
